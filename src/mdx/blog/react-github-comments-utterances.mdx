...
path: /blog/react-gitHub-comments-utterances
date: 2019-04-16
category: Tech
imageLink: https://publiuslogic.com/assets/utterances.jpg
tags: [React, Utterances, Comments]
publicId: videos/angelina_jordan_suspicious_minds
videoTitle: Angelina Jordan Suspicious Minds
description: React GitHub Comments Utterances uses withUtterances React npm module. Built with React Hooks and Love
---

## React withUtterances

[Repo](https://github.com/khw1031/withUtterances)
...
```bash:title=Utterances-Comments-Install
...
```tsx:title=src/components/withUtterances.tsx {1,3,9,13,17,21}
...
```tsx:title=src/components/withUtterances.tsx {3,9,15}
...
```tsx:title=Without-Npm-Module {2,12-23}
import * as React from 'react'
...
```tsx:title=src/components/react-github-btn.tsx {1}
import GitHubButton from 'react-github-btn'
```

Then tell React that, `window !== undefined`.

```tsx:title=src/page/page.tsx {2}
// eslint-disable-next-line valid-typeof
if (typeof window !== undefined) {
  require('react-github-btn')
}
```

The Component for the issues button

```tsx:title=src/components/issues.js {1-2,5,10-15}
